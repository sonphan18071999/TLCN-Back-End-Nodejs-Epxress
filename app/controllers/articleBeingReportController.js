...
exports.createReports = async (req, res, next) => {
  ...
  var article = await db.articleBeingReportModels.findOne({
  ...
  var b = await db.articleBeingReportModels
    .findOneAndUpdate(
      { idArticle: req.body.idArticle },
...
exports.disableArticle = async (req, res, next) => {
  await db.articleBeingReportModels
    .findOneAndUpdate(
      { idArticle: req.body.idArticle },
      { $set: { isDisabled: true } }
    )
...
exports.setLevelForArticleReport = async (req, res, next) => {
  var level = await db.articleBeingReportModels.findOneAndUpdate(
    { idArticle: req.body.idArticle },
    { $set: { levelBan: req.body.level } }
  );
  if (level) {
