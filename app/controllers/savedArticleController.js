    ...
    if (user) {
      ...
      user.allArticleSaved.forEach((element) => {
        if (element.idArticle == data.allArticleSaved.idArticle) {
          user.save();
          user.allArticleSaved.pop({
            idArticle: data.allArticleSaved.idArticle,
          });
          flag = 1;
        }
      });
...
exports.getSavedArticleByIdUser = async (req, res, next) => {
  ...
  var allSavedIdArtile = await db.savedModels.findOne({
  ...
  if (!allSavedIdArtile) {
    //Nếu không có article nào đã lưu thì return.
    return res.status(500).json({
  ...
  for (var item of allSavedIdArtile.allArticleSaved) {
    if (count > req.body.count) {
      //Đã đạt số lượng bài viết yêu cầu
      // Trả về số article
      return res.status(200).json({
