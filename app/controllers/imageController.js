...
exports.getAllImage = async function (req, res, next) {
  var gfs = Grid(
    "mongodb+srv://sonp:Chikiet1@@clusterblogaccessories.w6uag.gcp.mongodb.net/<BlogAccessories>?retryWrites=true&w=majority",
    mongo
  );
  gfs
    .collection("photos.files")
    .findOne(
      { _id: "5f7f32e4cacd5c3a048c2a32", root: "resume" },
      function (err, file) {
        if (err) {
